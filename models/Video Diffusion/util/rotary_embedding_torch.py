...
'''Code from: https://github.com/lucidrains/rotary-embedding-torch '''
...
class RotaryEmbedding(nn.Module):
    ...
    def rotate_queries_and_keys(self, q, k, seq_dim = -2):
        ...
        scale = self.get_scale(lambda: seq, cache_key = seq_len, freqs=freqs)
        rotated_q = apply_rotary_emb(freqs, q, scale = scale)
        rotated_k = apply_rotary_emb(freqs, k, scale = scale ** -1)
        return rotated_q, rotated_k

    def get_scale(self, t, cache_key = None, freqs=None):
        assert self.use_xpos
